...
export const createTranstractionService = ({ api_key ,quantity,provider,id}) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (api_key === user.api_key) {
          if (user) {
            const userId =user.id
                ...
                const totalProduct = await Product.count({provider : provider , status:0});
                const products = await Product.find(
                { provider: provider, status: 0 },{ email: 1, password: 1}).limit(quantity);
        
                const productIds = products.map(product => product._id);
                 if (totalProduct >= quantityNum) {
                  await Transtraction.findByIdAndUpdate(newTrans.id, {status:1, products: products} );
                  await Product.updateMany({_id : { $in: productIds }} , {status: 1,tranId: newTrans.id} );
  ...
  export const updateTranstractionService = (transId) => {
    return new Promise(async (resolve, reject) => {
      try {
         ...
         const totalProduct = await Product.count({provider : trans.provider, status:0});
         const products = await Product.find(
          { provider: trans.provider, status: 0 },{ email: 1, password: 1 }).limit(trans.quantity);

          const productIds = products.map(product => product._id);
          if (totalProduct >= trans.quantity) {
            await Transtraction.findByIdAndUpdate(transId, {status:1,products: products} );
            await Product.updateMany({_id : { $in: productIds }} , {status:1,tranId: transId} );
